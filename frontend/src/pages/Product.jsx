...
//     {/* Submit Button */}
//     <button
//       className="bg-orange-500 text-white py-2 px-6 rounded-md cursor-pointer"
...
const Product = () => {
  ...
  useEffect(() => {
    ...
    if (product) {
      ...
      const savedRating = localStorage.getItem(`rating-${productId}`);
      setRating(savedRating ? Number(savedRating) : product.ratings?.rating || 0);
    }
  }, [productId, products]);
  ...
  const handleRatingSubmit = async (newRating) => {
    ...
    try {
      await axios.post(
        `${backendUrl}/api/product/rating`,
        { productId: productData._id, rating: newRating },
  ...
  const isInWishlist = productData ? wishlist.some(item => item._id === productData._id) : false;

  if (!productData) {
    return <div className="text-center text-gray-600 py-20">Loading product...</div>;
  }
  ...
  return (
    <div className="border-t-2 pt-10">
      ...
      <div className="flex gap-12 flex-col sm:flex-row">
        {/* Image Carousel */}
        <div className="flex-1 flex flex-col-reverse gap-3 sm:flex-row">
          <div className="flex sm:flex-col overflow-x-auto sm:overflow-y-scroll justify-between sm:w-[14%] w-full">
          ...
          <div className="w-full sm:w-[80%]">
            <img className="w-96" src={image} alt="main" />
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          <div className="mt-4">
            <StarRating currentRating={rating} onRatingSubmit={handleRatingSubmit} />
          </div>
            ...
            <div className="flex gap-2">
              {productData.sizes.map((item, index) => (
                <button
                  key={index}
                  onClick={() => setSize(item)}
                  className={`border py-2 px-4 bg-gray-100 rounded-lg ${item === size ? 'border-orange-500' : ''}`}
                >
                  {item}
                </button>
              ))}
          ...
          <div className="mt-4 flex gap-4">
            {isInWishlist ? (
              <button
                className="bg-red-500 py-3 px-8 text-white rounded-lg hover:bg-red-600"
                onClick={handleRemoveFromWishlist}
              >
                Remove from Wishlist
              </button>
              ...
              <button
                className="bg-orange-500 py-3 px-8 text-white rounded-lg hover:bg-orange-600"
                onClick={handleAddToWishlist}
              >
                Add to Wishlist
              </button>
      ...
      <div className="mt-10">
          ...
          {reviews.length ? (
            reviews.map((review, i) => (
              <div key={i} className="border p-4 rounded-lg shadow-sm">
                ...
                <p>{review.comment}</p>
                <span className="text-gray-400 text-sm">— {review.userdata || 'Anonymous'}</span>
              </div>
            ))
          ) : (
            <p className="text-gray-500">No reviews yet. Be the first to leave one!</p>
        ...
        <div className="mt-8">
          ...
          <div className="flex items-center gap-2 my-4">
            <StarRating currentRating={rating} onRatingSubmit={(newRating) => setRating(newRating)} />
            <span className="text-gray-500 text-sm">{rating ? `Your Rating: ${rating} stars` : 'Please select a rating'}</span>
          </div>
